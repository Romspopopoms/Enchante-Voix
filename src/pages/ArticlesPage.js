...
const ArticlePage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-40 xl:mt-36 gap-y-8">
      <h2 className='text-center text-3xl font-bold text-white mb-10'>News</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.length > 0 ? (
          articles.map((article, index) => (
            <div key={index} className={`bg-white p-4 rounded-lg shadow-lg flex flex-col items-center space-y-4 ${article.videoUrl || article.imageUrl ? 'col-span-1 lg:col-span-2' : 'col-span-1'}`}>
          ...
          <p className="text-white col-span-full">Aucun article trouvé.</p>
        )}
      </div>
    </div>
  );
